...
public class PerformerController {
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Delete a performer")
    ...
    public ResponseEntity<List<Performer>> getByCountry(@PathVariable String country) {
        return ResponseEntity.ok(performerService.findByCountryOfOrigin(country));
    }

    @GetMapping("/popular")
    @Operation(summary = "Get performers with popularity >= minPopularity")
    public ResponseEntity<List<Performer>> getPopular(@RequestParam Double minPopularity) {
        return ResponseEntity.ok(performerService.findByMinPopularity(minPopularity));
    }
}
